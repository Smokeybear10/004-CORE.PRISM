...
const DIM_META = {
  ...
  competitive:            { label: 'Competitive',sub: 'Rivals · Moats · Entrants',        color: '#D4B85A' },
...
function renderBeams(weights) {
  ...
  if (!svg) return;
  const W = 60, H = 520;
  const apexX = W - 1, apexY = H / 2;
  const baseX = 8, triTop = apexY - 70, triBot = apexY + 70;

  // Card centers — match the 5 grid rows on the right
  const cardHt = (H - 4 * 6) / 5;
  const targets = DIM_ORDER.map((key, i) => ({
    key,
    color: DIM_META[key].color,
    y: cardHt / 2 + i * (cardHt + 6),
  ...
  // Beams — thickness proportional to weight (range 1.5px → 7px)
  for (const t of targets) {
    const thickness = 1.5 + Math.max(0, Math.min(1, t.weight)) * 5.5;
    const opacity = weights ? (0.4 + Math.max(0, Math.min(1, t.weight)) * 0.55) : 0.5;
    html += `<line x1="${apexX}" y1="${apexY}"
                  x2="${W + 80}" y2="${t.y}"
...
function renderVerdict() {
  ...
  if (!verdict) {
    expWhy.innerHTML = 'Pick a flagged move on the chart above.';
  } else {
    expWhy.innerHTML = buildVerdictConclusion(STATE.selectedStrategy, verdict, STATE.lastFullStack);
  }
}
...
function applyAttributionResponse(move, response) {
  ...
  document.getElementById('canvas-meta').innerHTML =
    `<span class="num">${response.chunks_considered}</span> chunks · ` +
    `<span class="num">${response.enabled_sources.length}</span>/${ALL_SOURCE_IDS.length} sources · ` +
...
function applyPreBaked(bundle, moveIdx) {
  ...
  renderStrategyRow();
  document.getElementById('canvas-meta').innerHTML =
    `<span class="num">${attr.chunks_considered ?? '—'}</span> chunks · pre-baked · ` +
    `confidence <span class="num">${Math.round((attr.confidence ?? 0) * 100)}%</span>`;
}
